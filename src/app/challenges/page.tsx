...
const THEME_META: Record<string, { categoryColor: string; bgGradient: string }> = {
  ...
  robotics: {
    ...
    bgGradient: "from-(--primary)/20 to-[#102022]",
  },
  "healthcare ai": {
  ...
  default: {
    categoryColor: "bg-(--bg-elevated) text-(--text-base)",
    bgGradient: "from-(--bg-surface)/40 to-[#102022]",
  },
};
  ...
  return (
    <div className="group flex flex-col bg-(--bg-card) rounded-2xl overflow-hidden border border-(--border) hover:border-(--primary)/50 transition-all shadow-xl">
      ...
      >
        ...
        {status === "ended" && (
          <span className="absolute top-4 right-4 flex items-center gap-1.5 px-2 py-1 bg-(--text-faint)/20 border border-(--border)/40 text-(--text-muted) rounded-full text-[10px] font-bold uppercase">
            Ended
          </span>
        )}
      </div>
        ...
        <div className="flex gap-3">
          ...
          {status !== "ended" && (
            <Link
              ...
              className="flex-1 rounded-lg h-10 bg-(--primary) text-[#102022] text-sm font-bold flex items-center justify-center gap-1.5 hover:brightness-110 transition-all"
            >
    ...
    <div className="min-h-screen bg-(--bg-page) text-(--text-base) antialiased overflow-x-hidden">
      ...
      <main>
        ...
        <section className="relative h-130 flex items-center justify-center overflow-hidden">
          <div className="absolute inset-0 bg-linear-to-br from-(--bg-page) via-(--bg-sidebar) to-(--primary)/15" />
          {/* Grid overlay */}
          <div
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <a
                href="#challenges"
                className="rounded-xl h-14 px-8 bg-(--primary) text-[#102022] text-base font-black hover:shadow-[0_0_24px_rgba(56,189,248,0.4)] transition-all flex items-center gap-2"
              >
            ...
            {filters.map((f) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-bold border transition-all ${
                  activeFilter === f.val
                    ? "bg-(--primary) text-[#102022] border-(--primary)"
              ...
              {[
                ...
                >
                  <div className="relative">
                    <div className="size-20 bg-(--bg-page) border-4 border-(--primary)/20 group-hover:border-(--primary) flex items-center justify-center rounded-full mb-5 transition-all shadow-[0_0_20px_rgba(56,189,248,0.08)] group-hover:shadow-[0_0_24px_rgba(56,189,248,0.25)]">
                      <span className="material-symbols-outlined text-(--primary) text-4xl">
                        {icon}
                      </span>
                    </div>
                    <span className="absolute -top-2 -right-2 size-6 rounded-full bg-(--primary) text-[#102022] text-[10px] font-black flex items-center justify-center">
                      {step}
                    </span>
                  </div>
