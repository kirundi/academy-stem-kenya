...
const NOTIFICATIONS = [
  { label: "School updates", desc: "Changes to schools you are observing" },
  { label: "New reports available", desc: "When school performance reports are generated" },
  ...
  return (
    <div className="flex h-screen bg-(--bg-page) overflow-hidden">
      ...
      <main className="ml-60 flex-1 overflow-y-auto">
        <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.85)] backdrop-blur-md border-b border-[rgba(6,182,212,0.08)] px-6 py-3">
          <h1 className="text-lg font-bold text-(--text-base)">Settings</h1>
          <p className="text-(--text-muted) text-xs">Manage your observer account</p>
        </header>
            ...
            {activeTab === "account" && (
              <div className="bg-(--bg-card) rounded-xl border border-(--border-subtle) p-7">
                ...
                <div className="mt-5 p-4 rounded-xl border" style={{ background: `${ACCENT}10`, borderColor: `${ACCENT}30` }}>
                  <p className="text-xs font-semibold mb-0.5" style={{ color: ACCENT }}>Observer Role — Read-only</p>
                  <p className="text-(--text-muted) text-xs">Observer accounts have read-only visibility into assigned schools. Contact an admin to update your school assignments.</p>
                </div>
              ...
              <div className="space-y-6">
                <div className="bg-(--bg-card) rounded-xl border border-(--border-subtle) p-7">
                  ...
                  <div className="flex items-center justify-between p-4 bg-(--bg-page)/50 rounded-xl border border-(--border)">
                    ...
                    <a href="/auth/change-password" className="px-4 py-2 bg-(--bg-card) border border-(--border) text-(--text-base) text-sm font-bold rounded-lg hover:border-[rgba(6,182,212,0.5)] transition-colors">
                      Change Password
                    </a>
                  </div>
                </div>
