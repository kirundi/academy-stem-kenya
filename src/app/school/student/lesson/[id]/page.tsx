  ...
  if (steps.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <Link href="/school/student/dashboard" className="text-[#13eca4] text-sm font-semibold hover:underline">Back to Courses</Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#10221c] flex flex-col">
      ...
      <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.9)] backdrop-blur-md border-b border-[rgba(19,236,164,0.08)] px-8 h-14 flex items-center gap-4">
        <Link href="/school/student/dashboard" className="text-slate-400 hover:text-white transition-colors">
          <span className="material-symbols-outlined text-[22px]">arrow_back</span>
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="text-white font-bold truncate">{courseTitle}</h1>
          <p className="text-slate-500 text-xs">Step {activeStep} of {steps.length}</p>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-72 bg-[#0d1f1a] border-r border-[rgba(19,236,164,0.08)] flex flex-col overflow-y-auto">
          ...
          <div className="mx-3 mt-3 p-3 bg-[rgba(19,236,164,0.05)] border border-[rgba(19,236,164,0.1)] rounded-xl">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">Think about your process as you work — your reflection answers will make your portfolio shine!</p>
          </div>
              ...
              return (
                ...
                >
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${
                    ...
                    {step.completed ? (
                      <span className="material-symbols-outlined text-[14px] text-[#10221c]">check</span>
                    ) : (
                      <span className="text-xs font-bold text-slate-400">{step.id}</span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium truncate ${isCurrent ? "text-white" : step.completed ? "text-slate-400" : "text-slate-500"}`}>
                      {step.title}
                    </p>
          ...
          {currentStep && (
            <div className="max-w-3xl mx-auto">
              ...
              <div className="flex items-start gap-3 mb-2 text-slate-400 text-sm">
                <Link href="/school/student/dashboard" className="hover:text-[#13eca4] transition-colors">My Courses</Link>
                <span>&rsaquo;</span>
              ...
              <div className="flex items-center gap-3 mb-6 mt-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${typeColors[currentStep.type] ?? "#13eca4"}18` }}>
                  <span className="material-symbols-outlined text-[22px]" style={{ color: typeColors[currentStep.type] ?? "#13eca4" }}>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold px-2 py-0.5 rounded uppercase tracking-wider"
                      style={{ background: `${typeColors[currentStep.type] ?? "#13eca4"}18`, color: typeColors[currentStep.type] ?? "#13eca4" }}>
                      {currentStep.type}
                    </span>
              ...
              {(currentStep.type === "Reading" || currentStep.type === "Video" || currentStep.type === "Hands-on" || currentStep.type === "Project" || currentStep.type === "Quiz") && (
                <div className="bg-[#1a2e27] rounded-2xl border border-[rgba(19,236,164,0.08)] overflow-hidden mb-8">
                  <div className="p-8">
                    <p className="text-slate-300 text-lg leading-relaxed mb-6">
                      {currentStep.content || "Complete this lesson step to continue your progress."}
                    </p>
              ...
              {currentStep.type === "Reflection" && (
                <div className="space-y-6 mb-8">
                  <div className="bg-[#1a2e27] rounded-2xl border border-[rgba(19,236,164,0.08)] p-7">
                    ...
                    <p className="text-slate-400 text-sm mb-7">Honest reflection helps you grow as a maker and problem-solver. Take your time with each question.</p>
                    <div className="space-y-7">
                      {[
                        { q: "What was the most challenging part of this project?", placeholder: "Describe the biggest obstacle you encountered..." },
                        { q: "How did you solve a problem you encountered?", placeholder: "What steps did you take to work through it..." },
                        { q: "What is one thing you would do differently next time?", placeholder: "If you could restart, what would you change..." },
                      ].map(({ q, placeholder }, i) => (
                        <div key={i}>
                          <label className="block text-white font-semibold text-sm mb-2">{i + 1}. {q}</label>
                          <textarea
                            rows={4}
                    ...
                    <div className="border-t border-[rgba(255,255,255,0.06)] mt-7 pt-6">
                      ...
                      <div className="flex items-center gap-2">
                        {[1, 2, 3, 4, 5].map((s) => (
                          ...
                          >
                            <span
                              ...
                              style={{
                                color: s <= (hoverStar || starRating) ? "#f59e0b" : "rgba(255,255,255,0.15)",
                                fontVariationSettings: s <= (hoverStar || starRating) ? "'FILL' 1" : "'FILL' 0",
                              }}
                            >
                              star
                            </span>
                        ...
                        {starRating > 0 && (
                          <span className="text-slate-400 text-sm ml-2">{starRating} out of 5 stars</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-6 mb-8">
                  <div className="bg-[rgba(19,236,164,0.04)] border border-[rgba(19,236,164,0.15)] rounded-2xl p-6">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="material-symbols-outlined text-[#13eca4] text-[24px]" style={{ fontVariationSettings: "'FILL' 1" }}>checklist</span>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        <label key={i} className="flex items-center gap-3 cursor-pointer group">
                          <input type="checkbox" className="w-5 h-5 rounded accent-[#13eca4] cursor-pointer" />
                  ...
                  <div className="bg-[#1a2e27] rounded-2xl border border-[rgba(19,236,164,0.08)] p-7">
                    ...
                    <p className="text-slate-400 text-sm mb-5">Add any final notes or attach a file link before submitting to your teacher for review.</p>
                    <div className="mb-5">
                      <label className="text-slate-400 text-xs font-semibold block mb-1.5 uppercase tracking-wide">Additional Notes (optional)</label>
                      <textarea
                        rows={4}
                    ...
                    <div className="mb-6">
                      <label className="text-slate-400 text-xs font-semibold block mb-1.5 uppercase tracking-wide">File / Link (optional)</label>
                      <input
                        type="url"
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  {steps.map((s) => (
                    <div key={s.id} className={`h-2 rounded-full transition-all cursor-pointer ${
                      s.id === activeStep ? "w-6 bg-[#13eca4]" : s.completed ? "w-2 bg-[#13eca4]/50" : "w-2 bg-white/15"
                ...
                {activeStep < steps.length ? (
                  ...
                  >
                    ...
                    <span className="material-symbols-outlined text-[20px]">{submitting ? "progress_activity" : "upload_file"}</span>
                  </button>
                )}
              </div>
            </div>
