...
const defaultSkillColor = "#2dd4bf";
...
export default function BadgesPage() {
  ...
  const skills = useMemo(() => {
    ...
    if (entries.length === 0) {
      return [
        { name: "Coding", value: 0, color: "var(--primary-green)" },
  ...
  function ringPoints(pct: number) {
    return axes
      .map((a) => polarToXY(a, R_MAX * pct))
      .map((p) => `${p.x},${p.y}`)
      .join(" ");
  }
  function dataPoints() {
    return axes
      .map((a, i) => polarToXY(a, R_MAX * (skills[i].value / 100)))
      .map((p) => `${p.x},${p.y}`)
      .join(" ");
  }
    ...
    <div className="min-h-screen bg-(--bg-page)">
      <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.8)] backdrop-blur-md border-b border-(--border-subtle) px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 bg-(--primary-green) text-[#0d1f1a] text-sm font-bold px-4 py-2 rounded-xl hover:bg-(--primary-green-dark) transition-colors">
      ...
      <div className="px-8 py-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-(--bg-card) border border-(--border-subtle) rounded-2xl p-6 relative overflow-hidden">
            <div className="absolute inset-0 bg-linear-to-br from-[rgba(45,212,191,0.03)] to-transparent pointer-events-none" />
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-(--text-base) font-bold text-lg">Skill Proficiency Map</h3>
              <div className="flex items-center gap-2 bg-[rgba(45,212,191,0.1)] px-3 py-1 rounded-full">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              <div className="relative shrink-0">
                <svg width="300" height="300" viewBox="0 0 360 360">
                  ...
                  />
                  {axes.map((a, i) => {
                    const pt = polarToXY(a, R_MAX * (skills[i].value / 100));
                    return (
                      <circle
                        key={i}
                        cx={pt.x}
                  ...
                  {axes.map((a, i) => {
                    ...
                    return (
                      <text
                        ...
                        fill={activeSkill === i ? skills[i].color : "rgba(255,255,255,0.55)"}
                        fontSize="11"
                        fontWeight="700"
                      >
                ...
                {skills.map((s, i) => (
                  ...
                  >
                    <div className="flex justify-between text-xs mb-1.5">
                      <span
                        className="font-bold"
                        style={{ color: activeSkill === i ? "white" : "rgb(148,163,184)" }}
                      >
                        {s.name}
                      </span>
          ...
          <div className="flex flex-col gap-5">
            ...
            >
              <p className="text-[#0a1a18] text-xs font-bold uppercase tracking-widest opacity-80">
                Current Level
              </p>
              <p className="text-[#0a1a18] text-2xl font-black mt-1 leading-tight">Level {level}</p>
              <div className="mt-4 space-y-2">
                <div className="flex justify-between text-xs font-bold text-[#0a1a18]">
                  <span>Badges</span>
                  <span>
                    {earnedBadges.length} / {earnedBadges.length + lockedBadges.length}
                  </span>
                </div>
                <div className="w-full h-3 bg-[rgba(10,26,24,0.2)] rounded-full overflow-hidden">
                  <div
                    className="h-full bg-[#0a1a18] rounded-full"
                    style={{
                      width:
                ...
                <p className="text-[10px] font-medium opacity-70 pt-1 italic text-[#0a1a18]">
                  Earn more badges to level up.
                </p>
              </div>
            </div>
          ...
          {earnedBadges.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
              {earnedBadges.map((b) => (
                <div
                  key={b.id}
                  className="bg-(--bg-card) border border-[rgba(45,212,191,0.06)] rounded-2xl p-4 flex flex-col items-center text-center hover:border-(--border-strong) hover:shadow-lg hover:shadow-[rgba(45,212,191,0.05)] transition-all cursor-pointer group"
                >
                  <div
                  ...
                  >
                    <span
                      ...
                      style={{ color: b.color, fontVariationSettings: "'FILL' 1" }}
                    >
                      {b.icon}
                    </span>
                  </div>
