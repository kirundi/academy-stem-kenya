            ...
            <div className="lg:col-span-9 space-y-6">
                ...
                <section className="bg-(--bg-card) border border-(--border) rounded-xl p-7">
                  ...
                  <div className="flex items-center gap-6 mb-7">
                    <div className="relative">
                      <div className="w-24 h-24 rounded-2xl bg-[rgba(19,218,236,0.15)] flex items-center justify-center border-2 border-(--border) text-primary text-3xl font-bold">
                    ...
                    <div>
                      <h4 className="font-bold text-(--text-base)">Profile Photo</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      ...
                      <input
                        className="w-full bg-(--bg-page) border border-(--border) rounded-lg text-(--text-base) px-4 py-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-[#13daec] outline-none transition-all"
                    ...
                    <div className="md:col-span-2 space-y-1.5">
                      ...
                      <textarea
                        className="w-full bg-(--bg-page) border border-(--border) rounded-lg text-(--text-base) px-4 py-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-[#13daec] outline-none transition-all min-h-25 resize-none"
                  ...
                  <div className="mt-7 flex justify-end">
                    <button
                      onClick={handleSave}
                      className="px-6 py-2.5 bg-primary text-white font-bold rounded-lg hover:opacity-90 transition-opacity flex items-center gap-2"
                    >
                      {saved ? (
                        <>
              ...
              {activeTab === "security" && (
                <section className="bg-(--bg-card) border border-(--border) rounded-xl p-7">
                  <div className="flex items-center gap-3 mb-7">
                    ...
                    <h3 className="text-xl font-bold text-(--text-base)">Security & Authentication</h3>
                  </div>

                  <div className="space-y-5">
                    {/* Password */}
                    <div className="flex flex-col md:flex-row md:items-center justify-between p-5 bg-(--bg-page)/50 rounded-xl border border-(--border) gap-4">
                      <div>
                        <h4 className="font-bold text-(--text-base)">Password</h4>
                        <p className="text-sm text-(--text-muted)">Last changed 3 months ago</p>
                      </div>
                      <button className="px-4 py-2 bg-(--bg-card) border border-(--border) text-(--text-base) text-sm font-bold rounded-lg hover:border-primary/50 transition-colors">
                        Reset Password
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between p-5 bg-(--bg-page)/50 rounded-xl border border-(--border) gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${twoFAEnabled ? "bg-primary" : "bg-slate-700"}`}
                        >
                          <span
                    ...
                    {/* Active Sessions */}
                    <div className="p-5 bg-(--bg-page)/50 rounded-xl border border-(--border)">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="font-bold text-(--text-base)">Active Sessions</h4>
                        ...
                        {[
                          ...
                          >
                            ...
                            {s.current ? (
                              ...
                              <button className="text-xs text-(--text-faint) hover:text-[#ef4444] transition-colors">
                                Revoke
                              </button>
                            )}
                          </div>
              ...
              {activeTab === "integrations" && (
                <section className="bg-(--bg-card) border border-(--border) rounded-xl p-7">
                  ...
                  {/* Google Classroom */}
                  <div className="border border-(--border) bg-(--bg-page)/50 rounded-xl overflow-hidden">
                    ...
                    {gcConnected && (
                      <div className="p-6 space-y-5">
                        <h5 className="text-xs font-bold text-(--text-muted) uppercase tracking-widest">
                          Sync Options
                        </h5>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          {[
                            ...
                            <div
                              key={opt.label}
                              className="flex items-center justify-between p-4 bg-(--bg-card) rounded-lg"
                            >
                              <div>
                                <p className="text-sm font-bold text-(--text-base)">{opt.label}</p>
                                <p className="text-xs text-(--text-muted) mt-0.5">{opt.desc}</p>
                              </div>
                              <button
                                onClick={opt.toggle}
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0 ml-4 ${opt.state ? "bg-primary" : "bg-slate-700"}`}
                              >
                                <span
                  ...
                  {/* More integrations */}
                  <div className="mt-5 p-5 border border-dashed border-(--border) rounded-xl text-center">
                    <span className="material-symbols-outlined text-3xl text-(--text-faint) mb-2 block">
                      add_circle
                    </span>
              ...
              {activeTab === "notifications" && (
                <section className="bg-(--bg-card) border border-(--border) rounded-xl p-7">
                  <div className="flex items-center gap-3 mb-7">
                    ...
                    <h3 className="text-xl font-bold text-(--text-base)">Notification Preferences</h3>
                  </div>
                        ...
                        return (
                          ...
                          >
                            <div>
                              ...
                              <p className="text-xs text-(--text-muted) mt-0.5">{labels[key].desc}</p>
                            </div>
                            <button
                              onClick={() =>
                                setNotifPrefs((prev) => ({ ...prev, [key]: !prev[key] }))
                              }
                              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0 ml-4 ${val ? "bg-primary" : "bg-slate-700"}`}
                            >
                              <span
