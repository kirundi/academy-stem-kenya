    ...
    <div className="flex flex-col h-screen overflow-hidden bg-[#0a1a16] text-white">
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-[rgba(19,236,164,0.2)] bg-[#0d1f1a] z-20 relative">
        <div className="flex items-center gap-8">
          <Link href="/school/student/challenges" className="flex items-center gap-2 text-[#13eca4]">
          ...
          <nav className="hidden md:flex items-center gap-5">
            <Link href="/school/student/dashboard" className="text-sm font-medium text-slate-400 hover:text-[#13eca4] transition-colors">Dashboard</Link>
            <span className="text-sm font-medium text-[#13eca4]">Active Challenge</span>
            <Link href="#" className="text-sm font-medium text-slate-400 hover:text-[#13eca4] transition-colors">Resources</Link>
          </nav>
        </div>

        {/* Persistent Timer */}
        <div className={`absolute left-1/2 -translate-x-1/2 flex items-center gap-3 px-6 py-2 rounded-full border shadow-lg ${
          timer.expired
            ? "bg-red-900/40 border-red-500/40 shadow-red-500/20"
            : "bg-[#1a2e30]/80 border-[rgba(19,236,164,0.3)] shadow-[rgba(19,236,164,0.1)]"
        }`}>
          <span className={`material-symbols-outlined ${timer.expired ? "text-red-400 animate-pulse" : "text-[#13eca4]"}`}>timer</span>
          <div className="flex items-baseline gap-1">
            <span className={`text-xl font-black tabular-nums tracking-tighter ${timer.expired ? "text-red-400 animate-pulse" : ""}`}>{timer.display}</span>
            <span className="text-[10px] uppercase font-bold text-slate-400">{timer.expired ? "Expired" : "Remaining"}</span>
          </div>
          {timer.expired && (
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {submitting ? (
              ...
              <><span>Submit Project</span><span className="material-symbols-outlined text-sm">send</span></>
            )}
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-60 border-r border-[rgba(19,236,164,0.1)] bg-[#0d1f1a] overflow-y-auto p-4 hidden lg:flex flex-col">
          <div className="mb-6">
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4">Milestones</h3>
              ...
              {milestones.map((m) => (
                ...
                >
                  <span className="material-symbols-outlined text-sm">
                    {m.status === "done" ? "check_circle" : m.status === "active" ? "radio_button_checked" : "radio_button_unchecked"}
                  </span>
                  <span className="text-sm font-medium">{m.id}. {m.label}</span>
                </div>
              ))}
            </nav>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-8 max-w-4xl mx-auto w-full">
          <div className="mb-8">
            <div className="flex items-center gap-2 text-[#13eca4] mb-2">
              <span className="text-xs font-bold uppercase tracking-widest">Active Challenge #42</span>
            </div>
            <h1 className="text-4xl font-black tracking-tight mb-4">Sustainable Energy Model</h1>
            <p className="text-slate-400 text-base leading-relaxed">
              Your mission is to design a resilient and renewable energy grid for a simulated coastal city of 50,000 residents. Balance cost, efficiency, and environmental impact.
            </p>
          </div>
            ...
            {([
              ...
              <button
                ...
                className={`pb-4 border-b-2 font-bold text-sm transition-colors ${
                  activeTab === tab.key ? "border-[#13eca4] text-[#13eca4]" : "border-transparent text-slate-500 hover:text-slate-300"
                }`}
              >
                {tab.label}
              </button>
            ...
            <div className="space-y-10">
              <section>
                ...
                <p className="text-slate-400 leading-relaxed mb-6">
                  Current power grids rely heavily on non-renewable sources that are vulnerable to extreme weather events. In this hackathon, you must incorporate at least three different types of renewable energy sources (Wind, Solar, Hydro, or Geothermal).
                </p>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {resources.map((r) => (
                    <div key={r.title} className="p-4 rounded-xl border border-[rgba(19,236,164,0.1)] bg-[#0d1f1a] hover:border-[#13eca4]/40 transition-colors flex items-start gap-4 group cursor-pointer">
                      <div className="p-2 rounded-lg bg-[#13eca4]/20 text-[#13eca4]">
                        <span className="material-symbols-outlined">{r.icon}</span>
                      </div>
                      <div>
                        <h4 className="font-bold text-sm text-white group-hover:text-[#13eca4] transition-colors">{r.title}</h4>
          ...
          {activeTab === "guidelines" && (
            <ul className="space-y-4">
              {rules.map((rule, i) => (
                <li key={i} className="flex gap-3 text-sm p-4 bg-[#0d1f1a] rounded-xl border border-[rgba(19,236,164,0.1)]">
                  <span className="material-symbols-outlined text-[#13eca4] text-xl shrink-0">verified</span>
        ...
        <aside className="w-72 border-l border-[rgba(19,236,164,0.1)] bg-[#0d1f1a] overflow-y-auto p-6 hidden xl:flex flex-col gap-8">
          ...
          <section>
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4 flex justify-between items-center">
              Team Members
              <span className="px-2 py-0.5 rounded-full bg-[#13eca4]/20 text-[#13eca4] text-[10px] font-bold">Team Delta</span>
            </h3>
            <div className="space-y-3">
              {team.map((m) => (
                <div key={m.name} className={`flex items-center justify-between ${!m.online ? "opacity-60" : ""}`}>
                  <div className="flex items-center gap-3">
                  ...
                  <span className={`size-2 rounded-full ${m.online ? "bg-green-500" : "bg-slate-500"}`} />
                </div>
              ))}
            </div>
          ...
          {/* Rules */}
          <section>
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4">Challenge Rules</h3>
            <ul className="space-y-3">
              {rules.map((rule, i) => (
                <li key={i} className="flex gap-3 text-sm">
                  <span className="material-symbols-outlined text-[#13eca4] text-lg shrink-0">verified</span>
          ...
          <div className="p-4 rounded-xl bg-[#13eca4]/5 border border-[#13eca4]/20">
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-3">Our mentors are online. Send a help request to get a quick guidance session.</p>
