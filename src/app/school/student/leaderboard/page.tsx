      ...
      <main className="ml-60 flex-1 overflow-y-auto">
        ...
        <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.8)] backdrop-blur-md border-b border-[rgba(45,212,191,0.1)] px-8 h-16 flex items-center justify-between">
          <div>
            ...
            <p className="text-(--text-muted) text-xs mt-0.5">Top students in your school</p>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[rgba(45,212,191,0.12)] border border-[rgba(45,212,191,0.2)]">
            ...
            <span className="text-(--primary-green) text-xs font-semibold">{students.length} students</span>
          </div>
        </header>
        ...
        <div className="px-8 py-8 space-y-6">
          {/* My rank card */}
          {myEntry && myRank > 0 && (
            <div className="p-5 bg-[rgba(45,212,191,0.08)] rounded-2xl border border-[rgba(45,212,191,0.25)] flex items-center gap-4">
              <div className="text-2xl font-black text-(--primary-green) w-10 text-center">
                {myRank <= 3 ? MEDAL[myRank] : `#${myRank}`}
              </div>
              <div className="w-10 h-10 rounded-full bg-linear-to-br from-(--primary-green) to-(--primary-green-dark) flex items-center justify-center text-[#10221c] font-bold text-sm shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span
                    ...
                    style={{ background: "rgba(45,212,191,0.15)", color: "var(--primary-green)" }}
                  >
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-(--primary-green) font-black text-xl">#{myRank}</p>
              </div>
            </div>
          )}
          ...
          {top50.length === 0 ? (
            ...
            <div className="bg-(--bg-card) rounded-2xl border border-[rgba(45,212,191,0.08)] overflow-hidden">
              <div className="px-6 py-4 border-b border-[rgba(45,212,191,0.08)] flex items-center gap-2">
                <span className="material-symbols-outlined text-(--primary-green) text-[20px]">stars</span>
                <h2 className="text-(--text-base) font-bold">Top 50 Students</h2>
                <span className="ml-auto text-xs text-(--text-faint)">All Time</span>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-4 px-6 py-4 transition-colors ${
                        isMe
                          ? "bg-[rgba(45,212,191,0.06)] border-l-2 border-(--primary-green)"
                    ...
                    >
                      ...
                      <div className="w-10 text-center shrink-0">
                        {rank <= 3 ? (
                          ...
                          <span className="text-(--text-faint) font-bold text-sm">#{rank}</span>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className={`text-sm font-semibold truncate ${isMe ? "text-(--primary-green)" : "text-(--text-base)"}`}>
                            {student.displayName}
                            {isMe && <span className="text-(--text-muted) font-normal ml-1">(You)</span>}
                          </p>
                          <span
                            className="text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0"
                            style={{ background: "rgba(45,212,191,0.12)", color: "var(--primary-green)" }}
                          >
                            Lv {level}
                          </span>
                        </div>
                        ...
                        <div className="mt-1.5 flex items-center gap-2">
                          <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${xpPct}%`,
                                background: isMe ? "#2dd4bf" : "rgba(45,212,191,0.4)",
                              }}
                            />
                          </div>
