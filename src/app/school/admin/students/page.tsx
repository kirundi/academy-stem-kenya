  ...
  return (
    <div className="min-h-screen bg-(--bg-page)">
      <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.9)] backdrop-blur-md border-b border-(--border-subtle) px-8 h-14 flex items-center justify-between">
        <h1 className="text-(--text-base) font-bold text-lg">Students</h1>
      ...
      <main className="max-w-7xl mx-auto px-8 py-8 space-y-8">
        <section className="flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-(--text-base) text-4xl font-black leading-tight">Student Management</h1>
            <p className="text-(--text-muted) mt-1 max-w-2xl">
              View and manage all enrolled students across {classrooms.length} classrooms.
            </p>
          </div>
        </section>
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            {
              label: "Avg. per Classroom",
              value: classrooms.length > 0 ? Math.round(students.length / classrooms.length) : 0,
              icon: "bar_chart",
              iconColor: "#8b5cf6",
            },
            {
            ...
            >
              ...
              <span className="text-(--text-base) text-3xl font-bold">{s.value.toLocaleString()}</span>
            </div>
          ))}
        </section>
                  ...
                  filtered.map((s) => (
                    <tr key={s.id} className="hover:bg-[rgba(19,236,164,0.02)] transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-primary-green/10 flex items-center justify-center text-primary-green font-bold text-sm">
                            {s.displayName
                              ?.split(" ")
                              .map((w) => w[0])
                              .join("")
                          ...
                          <span className="text-(--text-base) font-semibold">{s.displayName}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-(--text-muted)">{s.badges?.length ?? 0} badges</td>
                    </tr>
                  ))
                )}
              </tbody>
