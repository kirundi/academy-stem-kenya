      ...
      <div className="px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-(--bg-card) border border-(--border-subtle) rounded-2xl overflow-hidden">
              <div className="divide-y divide-[rgba(255,255,255,0.05)]">
                {activities.length === 0 ? (
                  <div className="p-8 text-center text-(--text-faint) text-sm">No recent activity</div>
                ) : (
                  activities.slice(0, 8).map((a) => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-full bg-[rgba(45,212,191,0.1)] flex items-center justify-center text-(--primary-green) font-bold text-sm shrink-0">
          ...
          <div className="space-y-6">
            {/* Maintenance */}
            {!dismissed && (
              <div className="bg-[rgba(45,212,191,0.05)] border border-[rgba(45,212,191,0.15)] p-6 rounded-2xl relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <button
                    ...
                    className="text-xs font-bold uppercase tracking-wider text-(--primary-green) border border-[rgba(45,212,191,0.3)] px-3 py-1.5 rounded-lg hover:bg-[rgba(45,212,191,0.08)] transition-colors"
                  >
                    Dismiss
                  </button>
                </div>
            ...
            <div className="bg-(--bg-card) border border-(--border-subtle) p-6 rounded-2xl">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button
                    key={q.label}
                    className="flex flex-col items-center justify-center p-3 rounded-xl bg-(--glass-bg) hover:ring-2 hover:ring-[rgba(45,212,191,0.4)] hover:bg-[rgba(45,212,191,0.05)] transition-all gap-2"
                  >
                    <span className="material-symbols-outlined text-(--primary-green) text-[22px]">
                      {q.icon}
                    </span>
                    <span className="text-xs font-medium text-(--text-base)">{q.label}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="bg-(--bg-card) border border-(--border-subtle) rounded-2xl p-6 w-full max-w-md shadow-2xl">
            {inviteResult ? (
              ...
              <div className="text-center">
                <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[rgba(45,212,191,0.1)] border border-[rgba(45,212,191,0.2)] mb-4">
                ...
                <div className="bg-(--glass-bg) border border-(--border-subtle) rounded-lg px-4 py-3 mb-5">
                  <p className="text-xs text-(--text-faint) mb-1 text-left">Invite link (valid 48h)</p>
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-(--input-bg) text-(--text-base) font-semibold text-sm hover:bg-[rgba(255,255,255,0.1)] transition-colors"
                >
                  Done
                </button>
              </div>
                ...
                <form onSubmit={handleInvite} className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-1">
                    <button
                      type="button"
                      onClick={closeInviteModal}
                      className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-(--input-bg) text-(--text-base) hover:bg-[rgba(255,255,255,0.1)] transition-colors"
                    >
                      Cancel
                    </button>
                    <button
