...
const deptColors: Record<string, string> = {
  Physics: "#13eca4", Robotics: "#3b82f6", Biotech: "#8b5cf6",
  "Comp Sci": "#13eca4", "Game Design": "#ec4899", "Green Tech": "#10b981",
  "Data Sci": "#f59e0b", Cybersec: "#06b6d4", default: "#13eca4",
};

export default function TeacherManagementPage() {
  const [search, setSearch] = useState("");
  const [openMenu, setOpenMenu] = useState<number | null>(null);
  const { teachers, loading } = useSchoolAdminData();
    ...
    <div className="min-h-screen bg-[#10221c]">
      ...
      <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.9)] backdrop-blur-md border-b border-[rgba(19,236,164,0.08)] px-8 h-14 flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            ...
            <a href="/school/admin/classrooms" className="text-slate-400 hover:text-[#13eca4] font-medium transition-colors">Curriculum</a>
            <a href="/school/admin/students" className="text-slate-400 hover:text-[#13eca4] font-medium transition-colors">Students</a>
            <a href="/school/admin/analytics" className="text-slate-400 hover:text-[#13eca4] font-medium transition-colors">Analytics</a>
          </nav>
        </div>
        <div className="flex items-center gap-3">
      ...
      <main className="max-w-7xl mx-auto px-8 py-8 space-y-8">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { label: "Total Teachers",   value: String(teachers.length),   badge: `${teachers.length} on platform`, badgeColor: "text-emerald-500", icon: "groups",        iconColor: "#13eca4" },
            { label: "Active Classes",   value: "--",  badge: "Across departments", badgeColor: "text-slate-400",  icon: "class",         iconColor: "#13eca4" },
            { label: "Pending Invites",  value: "--",    badge: "Requires follow-up", badgeColor: "text-amber-500", icon: "mail",          iconColor: "#f59e0b" },
            { label: "Staff Attendance", value: "--", badge: "--",    badgeColor: "text-emerald-500", icon: "verified_user", iconColor: "#13eca4" },
          ].map((s) => (
              ...
              <tbody className="divide-y divide-[rgba(255,255,255,0.04)] text-sm">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((t, i) => {
                    ...
                    const status = "active";
                    return (
                      <tr key={t.id} className="hover:bg-[rgba(19,236,164,0.02)] transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-tight" style={{ background: `${deptColor}15`, color: deptColor }}>
                            {dept}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-slate-300 font-medium">{t.subjects?.join(", ") ?? "--"}</td>
                        <td className="px-6 py-4">
                            ...
                            <div className="relative">
                              ...
                              {openMenu === i && (
                                <div className="absolute right-0 top-full mt-1 w-44 bg-[#1a2e27] border border-[rgba(255,255,255,0.1)] rounded-xl shadow-2xl z-20 overflow-hidden">
                                  ...
                                  <button
                                    onClick={() => { remove(t.id); setOpenMenu(null); }}
