      ...
      <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.9)] backdrop-blur-md border-b border-[rgba(19,236,164,0.08)] px-8 h-14 flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            <a href="/school/admin" className="text-slate-400 hover:text-[#13eca4] font-medium transition-colors">Dashboard</a>
            <span className="text-[#13eca4] font-semibold border-b-2 border-[#13eca4] pb-0.5">Staff Management</span>
            <a href="/dashboard/content" className="text-slate-400 hover:text-[#13eca4] font-medium transition-colors">Curriculum</a>
            <a href="/dashboard/users" className="text-slate-400 hover:text-[#13eca4] font-medium transition-colors">Students</a>
            <a href="/dashboard/analytics" className="text-slate-400 hover:text-[#13eca4] font-medium transition-colors">Analytics</a>
          </nav>
        </div>
        <div className="flex items-center gap-3">
      ...
      <main className="max-w-7xl mx-auto px-8 py-8 space-y-8">
        ...
        <section className="flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-400 mt-1 max-w-2xl">Manage your educational staff, oversee department performance, and invite new educators to the STEM Academy platform.</p>
          </div>
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { label: "Total Teachers",   value: "42",   badge: "+3 this month", badgeColor: "text-emerald-500", icon: "groups",        iconColor: "#13eca4" },
            { label: "Active Classes",   value: "156",  badge: "Across 8 subjects", badgeColor: "text-slate-400",  icon: "class",         iconColor: "#13eca4" },
            { label: "Pending Invites",  value: "8",    badge: "Requires follow-up", badgeColor: "text-amber-500", icon: "mail",          iconColor: "#f59e0b" },
            { label: "Staff Attendance", value: "98.4%", badge: "Excellent",    badgeColor: "text-emerald-500", icon: "verified_user", iconColor: "#13eca4" },
          ].map((s) => (
        ...
        <section className="bg-[#1a2e27] rounded-2xl border border-[rgba(19,236,164,0.07)] overflow-hidden">
          <div className="px-6 py-5 border-b border-[rgba(255,255,255,0.06)] flex flex-wrap items-center justify-between gap-4">
            ...
            <span className="text-slate-500 text-sm">Showing {filtered.length} of {teachers.length} teachers</span>
          </div>
                ...
                {filtered.map((t, i) => (
                  <tr key={t.email} className="hover:bg-[rgba(19,236,164,0.02)] transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm ${t.invited ? "border border-dashed border-[rgba(255,255,255,0.15)]" : "bg-[rgba(19,236,164,0.1)] text-[#13eca4]"}`}>
                          <span className="material-symbols-outlined text-[18px]">{t.invited ? "person" : "person"}</span>
                        </div>
                        <div>
                          <p className={`font-bold ${t.invited ? "text-slate-500 italic" : "text-white"}`}>{t.name}</p>
                          <p className="text-xs text-slate-500">{t.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-tight" style={{ background: `${t.deptColor}15`, color: t.deptColor }}>
                        {t.dept}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className={`flex items-center gap-2 ${statusBadge[t.status as keyof typeof statusBadge]}`}>
                        <span className={`w-2 h-2 rounded-full ${statusDot[t.status as keyof typeof statusDot]} ${t.status === "invited" ? "animate-pulse" : ""}`} />
                          ...
                          <div className="relative">
                            ...
                            {openMenu === i && (
                              <div className="absolute right-0 top-full mt-1 w-44 bg-[#1a2e27] border border-[rgba(255,255,255,0.1)] rounded-xl shadow-2xl z-20 overflow-hidden">
                                <button className="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:bg-[rgba(255,255,255,0.05)] transition-colors">Edit Profile</button>
                                <button className="w-full text-left px-4 py-2.5 text-sm text-red-400 font-bold hover:bg-[rgba(239,68,68,0.08)] transition-colors">Revoke Access</button>
                              </div>
                            )}
                          </div>
                        </div>
