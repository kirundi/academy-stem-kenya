    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-(--bg-page)">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-(--text-muted) text-sm">Loading facilitation notes...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="relative flex min-h-screen flex-col bg-(--bg-page) text-(--text-base) overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-[rgba(255,255,255,0.07)] bg-[rgba(16,34,28,0.9)] backdrop-blur-md px-6 lg:px-10 py-3">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex items-center gap-1 h-10 bg-(--bg-card) border border-(--border-subtle) rounded-lg overflow-hidden">
            <div className="flex items-center pl-3 text-(--text-faint)">
              <span className="material-symbols-outlined text-xl">search</span>
            </div>
            <input
              placeholder="Search resources..."
              className="bg-transparent border-none focus:ring-0 text-sm text-(--text-base) placeholder:text-(--text-faint) px-3 py-2 outline-none w-48"
            />
          </div>
        </div>
        <div className="flex items-center gap-6">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center justify-center h-10 px-4 rounded-lg bg-(--primary-green) text-[#10221c] text-sm font-bold hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {savingNotes ? (
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="px-6 lg:px-10 py-6 border-b border-[rgba(255,255,255,0.07)]">
          <nav className="flex flex-wrap gap-2 mb-4 text-sm">
            ...
            <span className="text-(--text-faint)">/</span>
            <span className="text-(--text-base) font-medium">Step 3: Facilitation Notes</span>
          </nav>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div className="max-w-2xl">
              <h1 className="text-(--text-base) text-3xl font-black tracking-tight mb-2">
                Teacher Facilitation Notes
              </h1>
              <p className="text-(--text-muted) text-base">
                Add educator-only tips, classroom management strategies, and answer keys for this
                specific learning step.
              </p>
            </div>
            <Link
              href={`/dashboard/courses/create/preview?courseId=${courseId}`}
              className="flex items-center gap-2 h-10 px-4 rounded-lg bg-(--bg-card) text-(--text-base) text-sm font-bold border border-(--border-subtle) hover:bg-(--bg-elevated) transition-colors"
            >
            ...
            {[
              {
                ...
                num: "1",
                active: false,
                href: `/dashboard/courses/create/step2?courseId=${courseId}`,
              },
              {
        ...
        <div className="flex flex-1 overflow-hidden" style={{ minHeight: "600px" }}>
          ...
          <section className="flex-1 flex flex-col border-r border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.01)] overflow-y-auto">
            <div className="sticky top-0 z-10 p-4 border-b border-[rgba(255,255,255,0.07)] bg-[rgba(16,34,28,0.8)] backdrop-blur-sm flex justify-between items-center">
              <h3 className="text-xs font-bold uppercase tracking-wider text-(--text-faint) flex items-center gap-2">
                <span className="material-symbols-outlined text-sm">school</span>
                Student View (Read-Only)
              </h3>
              <div className="flex items-center gap-2 bg-(--input-bg) px-2 py-1 rounded">
                <span className="w-2 h-2 rounded-full bg-emerald-400" />
                <span className="text-[10px] font-bold text-(--text-faint) uppercase">Live Content</span>
              </div>
            </div>

            <div className="p-8 max-w-3xl mx-auto w-full">
              <div className="aspect-video w-full rounded-xl overflow-hidden mb-6 bg-(--bg-card) border border-(--border-subtle) relative group flex items-center justify-center">
                <span className="material-symbols-outlined text-(--text-faint) text-7xl">
                  play_circle
                </span>
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-[rgba(16,34,28,0.5)]">
                  <span className="material-symbols-outlined text-7xl text-(--text-base)">play_circle</span>
                </div>
              </div>
              <h2 className="text-2xl font-bold mb-4">
                {course?.title || "Module 3: Wiring the Controller"}
              </h2>
              ...
              <div className="bg-[rgba(45,212,191,0.06)] border-l-4 border-(--primary-green) p-4 rounded-r-lg mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-(--text-muted)">
                      Always double-check your ground connections to prevent short circuits.
                    </p>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-(--border-subtle) rounded-lg bg-(--bg-card)">
                  ...
                  <ul className="text-xs text-(--text-faint) mt-2 space-y-1">
          ...
          <section className="flex-1 flex flex-col bg-(--bg-page) overflow-y-auto">
            <div className="sticky top-0 z-10 p-4 border-b border-[rgba(255,255,255,0.07)] bg-[rgba(16,34,28,0.9)] flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="w-10 h-5 bg-[rgba(45,212,191,0.15)] rounded-full relative cursor-pointer border border-(--border-strong)">
            ...
            <div className="p-8 flex flex-col gap-6">
              <div>
                ...
                <div className="rounded-lg border border-(--border-subtle) overflow-hidden flex flex-col min-h-80">
                  {/* Toolbar */}
                  <div className="bg-(--bg-card) p-2 border-b border-(--border-subtle) flex gap-1 flex-wrap">
                    {[
                      "format_bold",
                      "format_italic",
                      ...
                      <button
                        key={icon}
                        className="p-1.5 hover:bg-[rgba(255,255,255,0.1)] rounded transition-colors text-(--text-muted) hover:text-(--text-base)"
                      >
                    ...
                    <button className="p-1.5 hover:bg-[rgba(255,255,255,0.1)] rounded transition-colors text-(--text-muted) hover:text-(--text-base)">
                  ...
                  <textarea
                    ...
                    className="flex-1 p-6 bg-[rgba(255,255,255,0.02)] focus:ring-0 border-none text-base leading-relaxed placeholder:text-(--text-faint) resize-none text-(--text-base) outline-none min-h-64"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div
                    ...
                    className="p-5 rounded-xl bg-(--bg-card) border-2 border-dashed border-(--border-subtle) flex flex-col items-center justify-center gap-2 text-center py-8 cursor-pointer hover:border-[rgba(45,212,191,0.4)] hover:bg-[rgba(45,212,191,0.03)] transition-all"
                  >
                    {uploading ? (
                      <>
                      ...
                      <>
                        ...
                        <p className="text-[10px] text-(--text-faint)">
                          PDF, PPTX, or ZIP files (Max 20MB)
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  {uploadedResources.length > 0 && (
                    <div className="mt-3 space-y-2">
                      {uploadedResources.map((res, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-3 p-2 rounded-lg bg-[rgba(45,212,191,0.04)] border border-(--border-subtle)"
                        >
                          <span className="material-symbols-outlined text-(--primary-green) text-sm">
                            description
                          </span>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-(--text-base) truncate">{res.name}</p>
                            <p className="text-[10px] text-(--text-faint)">{res.size}</p>
                          </div>
                          <a
                            href={res.url}
                            target="_blank"
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-[rgba(45,212,191,0.04)] border border-(--border-subtle)">
                    <div className="w-8 h-8 rounded-full bg-[rgba(45,212,191,0.15)] flex items-center justify-center text-(--primary-green) font-bold text-xs shrink-0">
                      JK
                    </div>
                    <div>
                      <p className="text-[11px] font-bold text-(--text-base)">
                        Mrs. Jenkins{" "}
                        <span className="text-(--text-muted) font-normal">added a note:</span>
                      </p>
                      <p className="text-[11px] italic text-(--text-muted)">
                        &quot;My period 4 class struggled with the blue wire length.&quot;
                      </p>
                    </div>
                  </div>
                </div>
      ...
      {/* Footer */}
      <footer className="border-t border-[rgba(255,255,255,0.07)] bg-(--bg-page) p-4 flex justify-between items-center px-10 shrink-0">
        ...
        <div className="flex gap-4">
          <Link
            href={`/dashboard/courses/create/step2?courseId=${courseId}`}
            className="px-6 py-2 rounded-lg text-(--text-muted) font-bold text-sm hover:bg-(--input-bg) transition-colors"
          >
            Back to Curriculum
          </Link>
          <Link
            href={`/dashboard/courses/create/step4?courseId=${courseId}`}
            className="px-8 py-2 rounded-lg bg-(--primary-green) text-[#10221c] font-black text-sm shadow-lg shadow-[rgba(45,212,191,0.15)] hover:opacity-90 transition-opacity"
          >
            Next: Review &amp; Publish
          </Link>
        </div>
