...
type DocSection = "getting-started" | "authentication" | "google-classroom" | "webhooks" | "rate-limits" | "error-codes";
...
const ENDPOINTS: Endpoint[] = [
  {
    ...
    description: "Retrieve the current synchronization status for a specific school or district. Returns the timestamp of the last successful sync and any pending tasks.",
    params: [
      { name: "school_id", type: "string", required: true, description: "The unique identifier for the school entity." },
      { name: "include_logs", type: "boolean", required: false, description: "Whether to include the last 5 sync event logs." },
    ],
    responseExample: `{
  "status": "success",
  "data": {
  ...
  },
  {
    ...
    description: "Manually trigger a full synchronization cycle for the specified school. This will queue a roster sync, grade passback, and assignment pull.",
    params: [
      { name: "school_id", type: "string", required: true, description: "The unique identifier for the school entity." },
      { name: "sync_type", type: "string", required: false, description: 'One of: "roster", "grades", "assignments", "full". Defaults to "full".' },
    ],
    responseExample: `{
  "status": "accepted",
  "data": {
  ...
  },
  {
    ...
    description: "Disconnect a school's Google Classroom integration. This revokes OAuth tokens and halts all sync jobs. This action is irreversible without re-authorisation.",
    params: [
      { name: "school_id", type: "string", required: true, description: "The unique identifier for the school entity." },
      { name: "confirm", type: "boolean", required: true, description: 'Must be set to "true" to confirm the destructive action.' },
    ],
    responseExample: `{
  "status": "success",
...
const CHANGELOG = [
  { version: "v2.4.0", date: "Jan 24, 2024", changes: ["Added `sync_type` parameter to trigger endpoint", "Rate limit headers now included in all responses", "Deprecated `legacy_sync` flag removed"] },
  { version: "v2.3.1", date: "Dec 10, 2023", changes: ["Fixed race condition in roster update tasks", "Improved error messages for 404 responses", "Added `estimated_duration` to trigger response"] },
  { version: "v2.3.0", date: "Nov 1, 2023", changes: ["Introduced Google Classroom Sync API", "OAuth 2.0 PKCE flow for school-level auth", "Webhook delivery now supports retry with exponential backoff"] },
];
...
export default function DeveloperPortalPage() {
  ...
  const navItems: { section: DocSection; label: string; icon: string; group?: string }[] = [
    { section: "getting-started", label: "Getting Started", icon: "rocket_launch", group: "Guides" },
    { section: "authentication", label: "Authentication", icon: "vpn_key" },
    { section: "google-classroom", label: "Google Classroom Sync", icon: "sync_alt", group: "Integrations" },
    ...
    <div className="min-h-screen bg-[#102022] flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-[#2d4548] bg-[#102022]">
        <div className="flex items-center gap-3">
          ...
          <span className="px-2 py-0.5 bg-[#13daec]/10 text-[#13daec] text-[10px] font-bold rounded uppercase tracking-wider">API v2.4.0</span>
        </div>
        <div className="flex items-center gap-3">
        ...
        <aside className="w-64 border-r border-[#2d4548] bg-[#102022] flex flex-col overflow-y-auto shrink-0">
          <div className="p-5">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="px-2 py-1 rounded bg-[#13daec]/10 text-[#13daec] text-[10px] font-bold uppercase tracking-wider">Public</span>
            </div>
              ...
              {navItems.map((item, i) => {
                ...
                return (
                  <div key={item.section}>
                    ...
                    >
                      <span className={`material-symbols-outlined text-xl ${activeSection !== item.section ? "group-hover:text-[#13daec]" : ""}`}>
                        {item.icon}
                      </span>
                      <span className={`text-sm ${activeSection === item.section ? "font-bold" : "font-medium"}`}>{item.label}</span>
                    </button>
                  </div>
                );
              })}
        ...
        <main className="flex-1 overflow-y-auto bg-[#102022] p-8">
          <div className="max-w-5xl mx-auto">
            ...
            <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6">
              <button onClick={() => {}} className="hover:text-[#13daec]">Developer Portal</button>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-10 border-b border-[#2d4548]">
              <div className="space-y-3">
                <h1 className="text-white text-4xl font-extrabold tracking-tight">Google Classroom Sync API</h1>
                <p className="text-slate-400 text-lg max-w-2xl leading-relaxed">
                  The Sync API allows schools to automate the synchronization of rosters, assignments, and grades between STEM Impact Academy and Google Classroom.
                </p>
                <div className="flex items-center gap-3 pt-1">
                  <span className="px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-bold border border-emerald-500/20">Stable</span>
            ...
            {activeTab === "overview" && (
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                    { icon: "sync_alt", label: "Roster Sync", desc: "Automatically sync student and teacher rosters from Google Classroom to STEM Academy on a configurable schedule." },
                    { icon: "grade", label: "Grade Passback", desc: "Push assignment scores from STEM Academy back to Google Classroom gradebooks in real-time." },
                    { icon: "assignment", label: "Assignment Pull", desc: "Import assignments and coursework from connected Google Classroom courses into the platform." },
                  ].map(({ icon, label, desc }) => (
                    <div key={label} className="p-5 rounded-xl bg-[#1a2e30] border border-[#2d4548]">
                      <span className="material-symbols-outlined text-[#13daec] text-3xl mb-3 block">{icon}</span>
                ...
                <div className="p-6 rounded-xl bg-[#1a2e30] border border-[#2d4548]">
                  ...
                  <div className="flex items-center gap-3 bg-[#102022] rounded-lg p-3 font-mono text-sm text-[#13daec]">
                    ...
                    <button onClick={() => handleCopy("https://api.stemimpactacademy.org/api/v2", "base")} className="ml-auto text-slate-500 hover:text-[#13daec]">
                      <span className="material-symbols-outlined text-base">{copied === "base" ? "check" : "content_copy"}</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "endpoints" && (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                  ...
                  {ENDPOINTS.map((endpoint, i) => (
                    <section key={i}>
                      <h3 className="text-white text-xl font-bold mb-3">
                        {endpoint.method === "GET" ? "Fetch Sync Status" : endpoint.method === "POST" ? "Trigger Manual Sync" : "Disconnect Integration"}
                      </h3>
                      <p className="text-slate-400 mb-5 leading-relaxed text-sm">{endpoint.description}</p>

                      {/* Endpoint Tag */}
                      <div className="flex items-center gap-3 bg-[#1a2e30] p-3 rounded-lg border border-[#2d4548] mb-5">
                        <span className={`px-2 py-1 text-[10px] font-bold rounded uppercase ${METHOD_COLORS[endpoint.method]}`}>
                          {endpoint.method}
                        </span>
                        <code className="text-[#13daec] font-mono text-sm break-all">{endpoint.path}</code>
                        <button
                          onClick={() => handleCopy(endpoint.path, `path-${i}`)}
                          className="ml-auto text-slate-500 hover:text-[#13daec] shrink-0"
                        >
                          <span className="material-symbols-outlined text-base">{copied === `path-${i}` ? "check" : "content_copy"}</span>
                        </button>
                      </div>

                      {/* Params Table */}
                      <h4 className="text-slate-100 text-xs font-bold uppercase tracking-widest mb-3">Parameters</h4>
                            ...
                            {endpoint.params.map((p) => (
                              <tr key={p.name}>
                                ...
                                <td className="px-4 py-3">
                                  <span className={`text-xs font-bold px-2 py-0.5 rounded ${p.required ? "bg-rose-500/10 text-rose-400" : "bg-slate-700 text-slate-400"}`}>
                      ...
                      {i === 0 && (
                        <>
                          <h4 className="text-slate-100 text-xs font-bold uppercase tracking-widest mt-6 mb-3">Response Status Codes</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {[
                              { code: "200", label: "OK", desc: "Successful request. Sync status object returned.", color: "emerald" },
                              { code: "401", label: "Unauthorized", desc: "API key is missing or invalid.", color: "amber" },
                              { code: "404", label: "Not Found", desc: "The specified school_id does not exist.", color: "rose" },
                              { code: "429", label: "Too Many Requests", desc: "Rate limit exceeded for this endpoint.", color: "rose" },
                            ].map(({ code, label, desc, color }) => (
                              <div key={code} className={`p-4 rounded-lg bg-${color}-500/5 border border-${color}-500/20 flex gap-4`}>
                                <span className={`text-${color}-500 font-bold font-mono`}>{code}</span>
                                <div>
                ...
                <div className="lg:col-span-5 space-y-6">
                  {ENDPOINTS.map((endpoint, i) => (
                    <div key={i} className="rounded-xl overflow-hidden bg-[#0a1516] border border-[#2d4548] shadow-2xl">
                      <div className="flex items-center justify-between px-4 py-3 bg-[#1a2e30] border-b border-[#2d4548]">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">JSON Response Example</span>
                        <button
                          onClick={() => handleCopy(endpoint.responseExample, `resp-${i}`)}
                          className="text-slate-500 hover:text-[#13daec] transition-colors"
                        >
                          <span className="material-symbols-outlined text-lg">{copied === `resp-${i}` ? "check" : "content_copy"}</span>
                        </button>
                      </div>
                      <div className="p-5 overflow-x-auto">
                        <pre className="text-sm font-mono leading-relaxed text-slate-300 whitespace-pre-wrap">{endpoint.responseExample}</pre>
                      </div>
                    </div>
                  ))}
                  ...
                  <div className="p-5 rounded-xl bg-[#1a2e30] border border-[#2d4548] flex items-center gap-4">
                    <div className="size-12 rounded-full bg-[#13daec]/10 flex items-center justify-center shrink-0">
                      <span className="material-symbols-outlined text-[#13daec] text-2xl">support_agent</span>
                    </div>
                    <div>
                      <h5 className="text-white text-sm font-bold">Need help integrating?</h5>
                      <p className="text-slate-500 text-xs">Contact our specialized developer support team for dedicated assistance.</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-5">
                  <div className="p-5 rounded-xl bg-[#1a2e30] border border-[#2d4548]">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      <span className="px-2 py-0.5 bg-emerald-500 text-background-dark text-[10px] font-bold rounded uppercase">GET</span>
                      <code className="text-[#13daec] font-mono text-xs">/api/v2/integrations/google-classroom/sync/:school_id</code>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-slate-400 text-xs font-bold uppercase mb-1.5">school_id *</label>
                        <input
                          type="text"
                          value={tryItSchoolId}
                      ...
                      <div>
                        <label className="block text-slate-400 text-xs font-bold uppercase mb-1.5">Authorization Header</label>
                        <input
                          type="password"
                          value={tryItAuth}
                ...
                <div className="rounded-xl overflow-hidden border border-[#13daec]/30 bg-[#13daec]/5">
                  <div className="p-4 border-b border-[#13daec]/20 flex items-center justify-between bg-[#13daec]/10">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-[#13daec] font-bold text-sm uppercase tracking-widest">Console Output</h4>
                    </div>
                    {tryItOutput && (
                      <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 text-[10px] font-bold">HTTP 200</span>
                    )}
                  </div>
                  <div className="p-5 min-h-70">
                    {tryItOutput ? (
                      <pre className="text-sm font-mono text-slate-300 leading-relaxed whitespace-pre-wrap">{tryItOutput}</pre>
                    ) : tryItLoading ? (
            ...
            {activeTab === "changelog" && (
              <div className="space-y-6">
                {CHANGELOG.map((entry) => (
                  <div key={entry.version} className="p-6 rounded-xl bg-[#1a2e30] border border-[#2d4548]">
                    ...
                    <ul className="space-y-2">
                      {entry.changes.map((change, i) => (
                        <li key={i} className="flex items-start gap-2 text-slate-300 text-sm">
                          <span className="material-symbols-outlined text-[#13daec] text-base mt-0.5">check_circle</span>
                          {change}
                        </li>
                      ))}
                    </ul>
