    ...
    <div className="min-h-screen bg-(--bg-page)">
      <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.8)] backdrop-blur-md border-b border-[rgba(45,212,191,0.08)] px-8 h-16 flex items-center justify-between">
        <div>
          ...
          <p className="text-(--text-muted) text-xs mt-0.5">{allUsers.length} users on the platform</p>
        </div>
      ...
      <div className="px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          <div className="bg-(--bg-card) p-5 rounded-2xl border border-(--border-subtle)">
            <span className="text-(--text-muted) text-sm font-medium">Total Users</span>
          ...
          {Object.entries(roleBadge).map(([key, label]) => (
            ...
            >
              <span className="text-(--text-muted) text-sm font-medium">{label}s</span>
        ...
        {/* Tab Switcher */}
        <div className="flex gap-1 bg-(--bg-card) p-1 rounded-xl border border-[rgba(45,212,191,0.08)] self-start">
          <button
          ...
          >
            ...
            {invites.filter((i) => i.status === "pending").length > 0 && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${
                activeTab === "invites" ? "bg-(--bg-page) text-(--primary-green)" : "bg-[rgba(45,212,191,0.15)] text-(--primary-green)"
              }`}>
        ...
        {/* Table */}
        <div className={`bg-(--bg-card) rounded-2xl border border-[rgba(45,212,191,0.08)] overflow-hidden ${activeTab !== "users" ? "hidden" : ""}`}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((u, i) => {
                  const color = roleColors[u.role] ?? "#2dd4bf";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center text-(--text-muted) text-xs">{joined}</td>
                      <td className="px-4 py-4 text-center text-(--text-muted) text-xs">{lastActive}</td>
                      <td className="px-4 py-4 text-right relative">
                        <div className="flex items-center justify-end gap-1">
                          <button className="p-2 hover:bg-[rgba(45,212,191,0.08)] rounded-lg text-(--text-muted) hover:text-(--primary-green) transition-colors">
                              ...
                              {openMenu === i && (
                                <div className="absolute right-0 top-full mt-1 w-52 bg-(--bg-card) border border-(--border-subtle) rounded-xl shadow-2xl z-20 overflow-hidden">
                                  <div className="px-4 py-2 border-b border-(--border-subtle)">
                                    <p className="text-(--text-faint) text-[10px] uppercase tracking-wider font-medium">
                                      Change Role
                                    </p>
                                  </div>
                                    ...
                                    <button
                                      ...
                                      className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center gap-2 ${
                                        u.role === role
                                          ...
                                          : "text-(--text-base) hover:bg-[rgba(255,255,255,0.05)]"
                                      }`}
                                    >
                                      <span
                                  ...
                                  {canManageUsers && (
                                    <div className="border-t border-(--border-subtle)">
                                      <button
                                        ...
                                        className="w-full text-left px-4 py-2.5 text-sm text-(--text-base) hover:bg-[rgba(255,255,255,0.05)] transition-colors flex items-center gap-2"
                                      >
          ...
          <div className="bg-(--bg-card) rounded-2xl border border-[rgba(45,212,191,0.08)] overflow-hidden">
            <div className="px-6 py-4 border-b border-(--border-subtle) flex items-center justify-between">
              ...
              <button
                ...
                className="p-2 rounded-lg text-(--text-muted) hover:text-(--primary-green) hover:bg-[rgba(45,212,191,0.08)] transition-colors disabled:opacity-50"
                title="Refresh"
              >
            ...
            {invitesLoading && invites.length === 0 ? (
              <div className="flex items-center justify-center py-16">
                <span className="material-symbols-outlined animate-spin text-3xl text-(--primary-green)">progress_activity</span>
              </div>
            ) : invites.length === 0 ? (
              <div className="py-16 text-center">
                <span className="material-symbols-outlined text-[48px] text-(--text-faint) block mb-3">mark_email_unread</span>
                <p className="text-(--text-base) font-semibold mb-1">No invites sent yet</p>
                <p className="text-(--text-faint) text-sm">Invite users with the button above.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {invites.map((inv, i) => {
                      const color = roleColors[inv.role] ?? "#2dd4bf";
                      ...
                      return (
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-(--text-faint) text-xs">{inv.email}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-4 text-center">
                          ...
                          <td className="px-4 py-4 text-center text-(--text-muted) text-xs">{inv.invitedByName}</td>
                            ...
                            <div className="flex items-center justify-end gap-1">
                              {inv.status !== "accepted" && (
                                <button
                                  ...
                                  className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-(--primary-green) hover:bg-[rgba(45,212,191,0.08)] transition-colors disabled:opacity-50"
                                  title="Resend invite"
                                >
                                  {isActioning ? (
      ...
      {showInvite && (
        ...
        >
          <div
            className="bg-(--bg-card) rounded-2xl border border-[rgba(45,212,191,0.12)] w-full max-w-md shadow-2xl"
          ...
          >
            <div className="px-6 py-5 border-b border-(--border-subtle) flex items-center justify-between">
              ...
              <button
                onClick={closeInviteModal}
                className="p-1.5 hover:bg-(--input-bg) rounded-lg text-(--text-muted) hover:text-(--text-base) transition-colors"
              >
            ...
            {inviteResult ? (
              <div className="px-6 py-6">
                <div className="text-center mb-5">
                  <div className="w-12 h-12 rounded-full bg-[rgba(45,212,191,0.1)] flex items-center justify-center mx-auto mb-3">
                ...
                <div className="bg-[rgba(255,255,255,0.03)] border border-(--border-subtle) rounded-xl p-4">
                  <label className="text-(--text-faint) text-[10px] uppercase tracking-wider font-medium">
                    Invite link (valid 48h)
                  </label>
                  <p className="text-(--primary-green) text-xs font-mono break-all mt-1">
                ...
                <button
                  onClick={closeInviteModal}
                  className="mt-5 w-full bg-(--primary-green) text-[#10221c] font-bold text-sm py-2.5 rounded-lg hover:opacity-90 transition-opacity"
                >
                  Done
                </button>
              </div>
              ...
              <div className="px-6 py-6 space-y-4">
                ...
                <div>
                  <label className="text-(--text-muted) text-xs font-medium block mb-1.5">Email</label>
                  <input
                    type="email"
                    value={inviteEmail}
                ...
                <div>
                  <label className="text-(--text-muted) text-xs font-medium block mb-1.5">Role</label>
                    ...
                    {inviteRoleOptions.map((r) => (
                      <button
                        ...
                        className={`flex-1 px-3 py-2 rounded-lg text-xs font-bold transition-colors border ${
                          inviteRole === r
                            ? "border-(--primary-green) bg-[rgba(45,212,191,0.08)] text-(--primary-green)"
                            : "border-(--border-subtle) text-(--text-muted) hover:text-(--text-base)"
                        }`}
                      >
                        {roleBadge[r]}
                      </button>
                ...
                {inviteRole === "admin" && isSuperAdmin && (
                  <div>
                    <button
                      ...
                      className="text-(--text-muted) text-xs font-medium hover:text-(--primary-green) transition-colors flex items-center gap-1"
                    >
                              ...
                              <label
                                ...
                                className={`flex items-center gap-2 p-2 rounded-lg border transition-colors cursor-pointer ${
                                  invitePermissions.includes(p)
                                    ? "border-[rgba(45,212,191,0.15)] bg-[rgba(45,212,191,0.03)]"
                                    : "border-transparent"
                ...
                {inviteError && (
                  <div className="bg-[rgba(255,77,77,0.08)] border border-[rgba(255,77,77,0.2)] rounded-lg px-4 py-3">
                    <p className="text-(--accent-red) text-sm">{inviteError}</p>
                    {inviteIs409 && (
                      <button
                        onClick={handleResendFrom409}
                        disabled={inviteLoading}
                        className="mt-2 flex items-center gap-1.5 text-xs font-bold text-(--primary-green) hover:opacity-80 transition-opacity disabled:opacity-50"
                      >
                        {inviteLoading ? (
                          <span className="material-symbols-outlined animate-spin text-[14px]">progress_activity</span>
                        ) : (
                ...
                <button
                  ...
                  className="w-full bg-(--primary-green) text-[#10221c] font-bold text-sm py-2.5 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {inviteLoading ? (
      ...
      {rolesUser && (
        ...
        >
          <div
            className="bg-(--bg-card) rounded-2xl border border-[rgba(45,212,191,0.12)] w-full max-w-lg shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="px-6 py-5 border-b border-(--border-subtle) flex items-center justify-between">
              <div>
                ...
                <p className="text-(--text-muted) text-xs mt-0.5">
                  ...
                  <span
                    className="font-bold"
                    style={{ color: roleColors[rolesUser.primaryRole] ?? "#2dd4bf" }}
                  >
            ...
            {rolesLoading ? (
              ...
              <div className="px-6 py-5">
                <p className="text-(--text-faint) text-xs mb-4 leading-relaxed">
                ...
                <div className="mb-4 p-3 rounded-xl bg-[rgba(255,255,255,0.02)] border border-(--border-subtle)">
                  <p className="text-(--text-faint) text-[10px] uppercase tracking-wider font-medium mb-2">
                    Primary Role (locked)
                  </p>
                  <span
                    className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full"
                    style={{
                      color: roleColors[rolesUser.primaryRole] ?? "#2dd4bf",
                      background: `${roleColors[rolesUser.primaryRole] ?? "#2dd4bf"}18`,
                    }}
                  >
                ...
                <p className="text-(--text-faint) text-[10px] uppercase tracking-wider font-medium mb-3">
                  Additional Roles
                </p>
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                          isChecked
                            ? "border-[rgba(45,212,191,0.25)] bg-[rgba(45,212,191,0.05)]"
                      ...
                      >
                        <input
                          ...
                          className="accent-(--primary-green) shrink-0"
                        />
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-(--text-faint) text-[10px] truncate">/{r.replace("_", "-")}</p>
                          </div>
                        </div>
                      </label>
                    );
                ...
                {rolesError && (
                  <div className="mt-3 bg-[rgba(255,77,77,0.08)] border border-[rgba(255,77,77,0.2)] rounded-lg px-4 py-2.5">
                    <p className="text-(--accent-red) text-sm">{rolesError}</p>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="px-6 py-4 border-t border-(--border-subtle) flex items-center gap-3">
              <button
                ...
                className="text-(--text-muted) text-xs font-medium hover:text-(--text-base) transition-colors"
              ...
              <button
                onClick={() => setRolesUser(null)}
                className="border border-(--border-subtle) text-(--text-base) font-semibold text-sm px-5 py-2 rounded-lg hover:bg-(--glass-bg) transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="bg-(--primary-green) text-[#10221c] font-bold text-sm px-5 py-2 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center gap-2"
              >
                {rolesSaving && (
          ...
          >
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-start gap-3 p-3 rounded-xl border transition-colors cursor-pointer ${
                          isChecked
                            ? "border-[rgba(45,212,191,0.2)] bg-[rgba(45,212,191,0.04)]"
                      ...
                      >
                        <input
                          ...
                          className="mt-0.5 accent-(--primary-green)"
                        />
                        <div className="flex-1">
            ...
            <div className="px-6 py-4 border-t border-(--border-subtle) flex items-center gap-3">
              ...
              <button
                ...
                disabled={permSaving}
                className="bg-(--primary-green) text-[#10221c] font-bold text-sm px-5 py-2 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50"
              >
        ...
        >
          ...
          >
              ...
              >
                <span
                  className={`material-symbols-outlined text-2xl ${
                    confirmAction.type === "delete" ? "text-(--accent-red)" : "text-amber-500"
                  }`}
                >
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setConfirmAction(null)}
                className="flex-1 border border-(--border-subtle) text-(--text-base) font-semibold text-sm py-2.5 rounded-lg hover:bg-(--glass-bg) transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className={`flex-1 font-bold text-sm py-2.5 rounded-lg transition-opacity disabled:opacity-50 ${
                  confirmAction.type === "delete"
                    ? "bg-(--accent-red) text-(--text-base) hover:opacity-90"
                    : "bg-(--primary-green) text-[#10221c] hover:opacity-90"
                }`}
              >
                {actionLoading
                  ? "Processing..."
          ...
          >
            <div className="px-6 py-5 border-b border-(--border-subtle) flex items-center justify-between">
              <div>
                ...
                <p className="text-(--text-muted) text-xs mt-0.5">Downloads a CSV file to your device.</p>
              </div>
              <button
                onClick={() => setShowExport(false)}
                className="p-1.5 hover:bg-(--input-bg) rounded-lg text-(--text-muted) hover:text-(--text-base) transition-colors"
            ...
            <div className="px-6 py-5 space-y-5">
              {/* Dataset */}
              <div>
                <label className="text-(--text-muted) text-xs font-medium block mb-2">Dataset</label>
                <div className="flex gap-2">
                ...
                <div>
                  <label className="text-(--text-muted) text-xs font-medium block mb-2">
                    ...
                    <button
                      ...
                      className="ml-2 text-(--primary-green) hover:opacity-70 transition-opacity"
                    >
              ...
              {(exportDataset === "users" || exportDataset === "both") && (
                <div>
                  <label className="text-(--text-muted) text-xs font-medium block mb-1.5">Filter by Role</label>
                  <select
              ...
              <div className="bg-[rgba(255,255,255,0.03)] border border-(--border-subtle) rounded-xl px-4 py-3 flex items-center gap-3">
                <span className="material-symbols-outlined text-(--primary-green) text-[20px]">table_chart</span>
                <div className="text-xs text-(--text-muted)">
                  {exportDataset === "users" && (
                    <>Exporting <span className="text-(--text-base) font-bold">{exportRoleFilter === "all" ? allUsers.length : allUsers.filter((u) => u.role === exportRoleFilter).length}</span> users · <span className="text-(--text-base) font-bold">{exportUserCols.length}</span> columns → <span className="text-(--primary-green) font-mono">users.csv</span></>
                  )}
                  {exportDataset === "invites" && (
                    <>Exporting <span className="text-(--text-base) font-bold">{invites.length > 0 ? invites.length : "all"}</span> invites · <span className="text-(--text-base) font-bold">{exportInviteCols.length}</span> columns → <span className="text-(--primary-green) font-mono">invites.csv</span></>
                  )}
                  {exportDataset === "both" && (
                    <>Two files: <span className="text-(--primary-green) font-mono">users.csv</span> + <span className="text-(--primary-green) font-mono">invites.csv</span></>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-(--border-subtle) flex items-center gap-3">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-(--primary-green) text-[#10221c] font-bold text-sm py-2 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <span className="material-symbols-outlined text-[18px]">download</span>
                Download CSV
              </button>
