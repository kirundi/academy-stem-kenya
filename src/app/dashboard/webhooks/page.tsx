...
export default function WebhooksPage() {
  ...
  return (
    <div className="p-6 space-y-8 min-h-screen">
      {/* Page Header */}
      <div className="flex flex-wrap justify-between items-end gap-4 border-b border-(--border-subtle) pb-5">
        <div>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-(--primary) hover:bg-(--primary)/90 text-(--text-base) px-5 py-2.5 rounded-lg font-bold text-sm transition-all shadow-lg shadow-[rgba(56,189,248,0.2)]"
        >
      ...
      {/* New Webhook Form */}
      {showNewForm && (
        <section className="bg-(--bg-card) rounded-xl border border-(--border-subtle) overflow-hidden shadow-sm">
          <div className="p-5 border-b border-(--border-subtle) bg-(--bg-elevated)/30 flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-(--bg-elevated) border border-(--border) rounded-lg text-sm px-4 py-2.5 text-(--text-base) focus:ring-(--primary) focus:border-(--primary) outline-none transition-all"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-(--bg-elevated) border border-(--border) rounded-lg text-sm px-3 py-2.5 text-(--text-base) focus:ring-(--primary) outline-none transition-all"
                  >
                ...
                {EVENTS.map((ev) => (
                  <label key={ev} className="flex items-center gap-3 cursor-pointer group">
                    <input
                      ...
                      className="rounded border-(--border) bg-(--bg-elevated) text-(--primary) focus:ring-(--primary) w-4 h-4 accent-(--primary)"
                    />
          ...
          <div className="px-6 pb-5 flex justify-end gap-3">
            ...
            <button
              ...
              className="px-6 py-2.5 bg-(--primary) text-[#102022] font-bold rounded-lg text-sm hover:opacity-90 transition-opacity"
            >
              Create Webhook
            </button>
          </div>
      ...
      <section className="space-y-4">
        <h3 className="text-(--text-base) text-xl font-bold flex items-center gap-2">
          ...
          Active Webhooks
          <span className="ml-2 px-2 py-0.5 bg-[rgba(56,189,248,0.1)] text-(--primary) text-xs font-bold rounded-full">
        ...
        <div className="overflow-hidden rounded-xl border border-(--border-subtle) bg-(--bg-card) shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-(--hover-subtle) border-b border-(--border)">
              ...
              <tbody className="divide-y divide-(--border)">
                {webhooks.map((wh) => (
                  <tr key={wh.id} className="hover:bg-(--bg-elevated)/30 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-1">
                        {wh.events.map((ev) => (
                          <span
                            key={ev}
                            className="bg-[rgba(56,189,248,0.1)] text-(--primary) text-[10px] font-bold px-2 py-0.5 rounded uppercase"
                          >
                            {ev}
                          </span>
                        ))}
                    ...
                    <td className="px-5 py-4 text-center">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${wh.status === "Active" ? "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20" : "bg-(--text-faint)/10 text-(--text-faint) border border-(--border)/20"}`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full mr-1.5 ${wh.status === "Active" ? "bg-emerald-500" : "bg-(--text-faint)"}`}
                        />
                        {wh.status}
                      </span>
                    </td>
          ...
          <div className="p-4 space-y-3">
            {deliveries.length === 0 ? (
              ...
              deliveries.map((d) => (
                <div
                  key={d.id}
                  className={`flex items-center justify-between p-3 rounded-lg bg-(--hover-subtle) border-l-4 ${d.status === "200 OK" ? "border-emerald-500" : d.status === "500 ERR" ? "border-[#ef4444]" : "border-amber-400"}`}
                >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-(--bg-elevated) hover:bg-(--primary) hover:text-(--text-base) text-(--text-muted) rounded text-xs font-bold transition-all"
                  >
