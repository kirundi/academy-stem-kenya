...
export default function AdminChallengeCreator() {
  ...
  return (
    <div className="min-h-screen bg-[#0a1a16] text-(--text-base)">
      {/* Top Nav */}
      <header className="sticky top-0 z-30 flex items-center justify-between px-8 py-4 border-b border-(--border-medium) bg-(--bg-page)">
        <div className="flex items-center gap-8">
          <span className="font-black text-lg tracking-tight">Challenge Creator</span>
          <nav className="flex items-center gap-1 bg-(--bg-card)/60 p-1 rounded-lg">
            {[
            ...
            <section className="bg-(--bg-page) rounded-2xl border border-(--border-subtle) p-8">
              ...
              <div className="flex items-start gap-6 p-4 bg-(--bg-card) rounded-xl border border-(--border-subtle)">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-(--text-muted)">Students can submit up to 15 minutes past the deadline with a point penalty.</p>
                  {lateSubmissions && (
                ...
                <button onClick={() => setLateSubmissions(!lateSubmissions)} className={`relative w-12 h-7 rounded-full transition-colors shrink-0 ${lateSubmissions ? "bg-(--primary-green)" : "bg-(--bg-elevated)"}`}>
          ...
          <div className="col-span-12 lg:col-span-4">
            <div className="sticky top-24 space-y-6">
              <div className="bg-(--bg-page) rounded-2xl border border-(--border-subtle) p-6">
                <h3 className="text-sm font-bold uppercase tracking-wider text-(--text-muted) mb-4">Participation Scope</h3>
                          ...
                          <div className="space-y-1.5 max-h-40 overflow-y-auto">
                            {filteredSchools.map((s) => (
                              <label key={s.id} className="flex items-center gap-2 cursor-pointer group">
                                <div className={`w-4 h-4 rounded border shrink-0 flex items-center justify-center transition-colors ${selectedSchools.includes(s.id) ? "bg-(--primary-green) border-(--primary-green)" : "border-(--border) group-hover:border-(--primary-green)/50"}`} onClick={() => toggleSchool(s.id)}>
                            ...
                            {filteredSchools.length === 0 && <p className="text-(--text-faint) text-xs">No schools found</p>}
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div className="bg-(--bg-page) rounded-2xl border border-(--border-subtle) p-5">
                ...
                <div className="rounded-xl bg-(--bg-card) overflow-hidden border border-(--border-subtle)">
                  <div className="h-20 bg-linear-to-r from-(--primary-green)/30 to-[#1a2e30] flex items-center px-4">
