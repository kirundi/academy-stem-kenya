...
const ACCENT = "#a855f7";
const ACCENT_BG = "rgba(168,85,247,0.1)";

const NOTIFICATIONS = [
  { label: "New report ready", desc: "When platform analytics reports are generated" },
  { label: "Data export complete", desc: "When a data export job finishes" },
  { label: "Platform updates", desc: "New features and system announcements" },
];
  ...
  return (
    <div className="flex h-screen bg-(--bg-page) overflow-hidden">
      ...
      <main className="ml-60 flex-1 overflow-y-auto">
        <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.85)] backdrop-blur-md border-b border-[rgba(168,85,247,0.08)] px-6 py-3">
          <h1 className="text-lg font-bold text-(--text-base)">Settings</h1>
          <p className="text-(--text-muted) text-xs">Manage your analytics account</p>
        </header>
            ...
            {activeTab === "account" && (
              <div className="bg-(--bg-card) rounded-xl border border-(--border-subtle) p-7">
                ...
                <div className="mt-5 p-4 rounded-xl border" style={{ background: `${ACCENT}10`, borderColor: `${ACCENT}30` }}>
                  <p className="text-xs font-semibold mb-0.5" style={{ color: ACCENT }}>Analytics Viewer — Read-only</p>
                  <p className="text-(--text-muted) text-xs">Analytics accounts have read-only access to platform metrics. Contact an admin to update your access scope.</p>
                </div>
              ...
              <div className="space-y-6">
                <div className="bg-(--bg-card) rounded-xl border border-(--border-subtle) p-7">
                  ...
                  <div className="flex items-center justify-between p-4 bg-(--bg-page)/50 rounded-xl border border-(--border)">
                    ...
                    <a href="/auth/change-password" className="px-4 py-2 bg-(--bg-card) border border-(--border) text-(--text-base) text-sm font-bold rounded-lg hover:border-[rgba(168,85,247,0.5)] transition-colors">
                      Change Password
                    </a>
                  </div>
                </div>
                <div className="bg-(--bg-card) rounded-xl border border-(--border-subtle) p-7">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={revokeAll} disabled={revoking === "all"} className="px-4 py-2 bg-accent-red/10 border border-[rgba(255,77,77,0.3)] text-accent-red text-sm font-bold rounded-lg hover:bg-[rgba(255,77,77,0.2)] transition-colors disabled:opacity-50">
