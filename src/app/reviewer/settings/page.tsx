...
const NOTIFICATIONS = [
  { label: "New course submitted", desc: "When an editor submits a course for review" },
  { label: "Review deadline", desc: "Reminders when review deadlines are approaching" },
  ...
  return (
    <div className="flex h-screen bg-(--bg-page) overflow-hidden">
      ...
      <main className="ml-60 flex-1 overflow-y-auto">
        <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.85)] backdrop-blur-md border-b border-(--border-subtle) px-6 py-3">
          <h1 className="text-lg font-bold text-(--text-base)">Settings</h1>
          <p className="text-(--text-muted) text-xs">Manage your reviewer account</p>
        </header>
            ...
            {activeTab === "account" && (
              <div className="bg-(--bg-card) rounded-xl border border-(--border-subtle) p-7">
                ...
                <div className="mt-5 p-4 rounded-xl border" style={{ background: `${ACCENT}10`, borderColor: `${ACCENT}30` }}>
                  <p className="text-xs font-semibold mb-0.5" style={{ color: ACCENT }}>Content Reviewer</p>
                  <p className="text-(--text-muted) text-xs">You review and approve courses submitted by editors before they are published to students.</p>
                </div>
