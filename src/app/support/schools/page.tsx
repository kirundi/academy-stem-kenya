  ...
  return (
    <div className="min-h-screen bg-(--bg-page)">
      <header className="sticky top-0 z-10 bg-[rgba(16,34,28,0.8)] backdrop-blur-md border-b border-[rgba(59,130,246,0.1)] px-8 h-16 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-(--text-base)">School Lookup</h1>
      ...
      <div className="px-8 py-8 max-w-5xl mx-auto">
        ...
        <div className="relative bg-linear-to-r from-[#1a2e27] to-[#162820] rounded-2xl p-8 mb-8 border border-[rgba(59,130,246,0.15)] overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-2xl font-bold text-(--text-base) mb-1">Search Schools</h2>
        ...
        <div className="bg-(--bg-card) rounded-2xl border border-[rgba(59,130,246,0.12)] overflow-hidden mb-6">
          <div className="p-6 border-b border-[rgba(59,130,246,0.08)]">
            ...
            <form onSubmit={handleSearch} className="flex gap-3">
              <div className="flex-1 relative">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-(--text-faint) text-[20px]">search</span>
                <input
                  type="text"
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Search by school name…"
                  className="w-full bg-(--input-bg) border border-(--border-subtle) rounded-xl pl-10 pr-4 py-3 text-(--text-base) placeholder:text-(--text-faint) text-sm focus:outline-none focus:border-[rgba(59,130,246,0.5)] transition-all"
                />
              </div>
              <button
                type="submit"
          ...
          {searched && results.length === 0 && (
            <div className="py-12 text-center">
              ...
              <p className="text-(--text-muted) text-sm">No schools found for &ldquo;{query}&rdquo;</p>
            </div>
          )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-(--text-muted) text-xs truncate">{school.county ?? "No county"} · {formatDate(school.createdAt)}</p>
                    </div>
                    <span
        ...
        {selected && (
          <div className="bg-(--bg-card) rounded-2xl border border-[rgba(59,130,246,0.15)] overflow-hidden">
            <div className="px-6 py-4 border-b border-[rgba(59,130,246,0.08)] flex items-center gap-3">
              ...
              <h2 className="text-(--text-base) font-bold">School Profile</h2>
            ...
            <div className="p-6">
              <div className="flex items-center gap-5 mb-6">
                ...
                <div>
                  <h3 className="text-(--text-base) font-bold text-lg">{selected.name}</h3>
                  {selected.email && <p className="text-(--text-muted) text-sm">{selected.email}</p>}
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label} className="p-4 bg-[rgba(255,255,255,0.02)] rounded-xl border border-(--border-subtle)">
                    ...
                    <p className={`text-(--text-base) text-sm ${mono ? "font-mono text-xs break-all" : "font-medium"}`}>{value}</p>
                  </div>
                ))}
              </div>
